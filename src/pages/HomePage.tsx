...
const HomePage = () => {
  ...
  const fetchContacts = async (page = 1) => {
    ...
    setIsLoading(true);
    try {
      const apiClient = getApiClient(token || ''); // Fix null issue
      const response = await apiClient.get(`/contacts?page=${page}&limit=${pagination.limit}`);
  ...
  const memoizedFetchContacts = useCallback(fetchContacts, [token, pagination.limit]);
  ...
  }, [token, navigate, memoizedFetchContacts]);

  // Fix event parameter typing
  ...
  // Fix newPage parameter typing
  const handlePageChange = (newPage: number) => {
    if (newPage > 0 && newPage <= pagination.totalPages) {
      memoizedFetchContacts(newPage);
    }
  };

  // Fix dateString parameter typing
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">No contacts found. Import a CSV file to get started.</p>
            </div>
          ) : (
            <>
                    ...
                    {contacts.map((contact) => (
                      <tr key={contact.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          {callStatus.contactId === contact.id && callStatus.message ? (
                            <span className={`inline-block px-3 py-1 rounded-full ${
                              callStatus.isError ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                            }`}>
                            ...
                            <button
                              ...
                              className={`text-green-600 hover:text-green-900 bg-green-100 hover:bg-green-200 px-3 py-1 rounded-full ${
                                callStatus.isLoading ? 'opacity-50 cursor-not-allowed' : ''
                              }`}
                            >
